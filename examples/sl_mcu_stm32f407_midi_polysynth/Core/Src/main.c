 ...
 * @attention
 *
 * USB MIDI Polyphonic Synthesizer
 *
...
void process_buffer(uint16_t *out_buffer) {
    ...
    float master_gain = (active_count > 0) ? (1.0f / sqrtf((float) active_count)) * 2000.0f * global_master_volume : 0.0f;
...
void synth_note_on(uint8_t note, uint8_t velocity) {
    ...
    // Fallback: simple voice stealing slot 0
    if (slot == -1) {
        slot = 0;
    }
...
int main(void)
{
    ...
    printf("\n\n\nStarting Polyphonic Synthesizer\n");
