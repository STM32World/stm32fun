...
static err_t ip_init_cb(struct netif *netif)
{
    ...
    // MANDATORY FLAGS for lwIP DHCP state machine:
    netif->flags = NETIF_FLAG_BROADCAST | NETIF_FLAG_ETHARP | NETIF_FLAG_LINK_UP | NETIF_FLAG_ETHERNET;
...
//        return true;
//
//    printf("RX Packet! len=%d\n", size); // Check if host output triggers this
//
//    struct pbuf *p = pbuf_alloc(PBUF_RAW, size, PBUF_POOL);
//    if (p) {
//        pbuf_take(p, src, size);
//
//        err_t err = netif_data.input(p, &netif_data);
//        if (err != ERR_OK) {
//            printf("netif.input failed: %d\n", err);
//            pbuf_free(p);
//        }
//    }
...
bool tud_network_recv_cb(const uint8_t *src, uint16_t size) {
    ...
    // Allocate pbuf with Ethernet link header space pre-reserved
    struct pbuf *p = pbuf_alloc(PBUF_LINK, size, PBUF_POOL);
    if (p) {
        pbuf_take(p, src, size);

        // Ensure input handler points to ethernet_input / etharp_input
        if (netif_data.input(p, &netif_data) != ERR_OK) {
            pbuf_free(p);
        }
    }
...
int main(void)
{
    ...
    lwip_init();
        ...
        // Pass 0.0.0.0 for IP, netmask, and gateway when using a DHCP client
        ip4_addr_t ipaddr, netmask, gw;
        ip4_addr_set_zero(&ipaddr);
        ip4_addr_set_zero(&netmask);
        ip4_addr_set_zero(&gw);

        // Add the network interface with unassigned addresses
        //netif_add(&netif_data, &ipaddr, &netmask, &gw, NULL, ip_init_cb, netif_input);
        netif_add(&netif_data, &ipaddr, &netmask, &gw, NULL, ip_init_cb, ethernet_input);
        netif_set_default(&netif_data);
        netif_set_up(&netif_data);
        netif_set_link_up(&netif_data);

        // Start the lwIP DHCP client (replaces dhcps_init)
        dhcp_start(&netif_data);

        // Initialize HTTP Server (will bind to 0.0.0.0 and respond once an IP is leased)
        httpd_init();
