...
#define TRNG ((volatile struct rng *) 0x50060800)
...
int main(void)
{
    ...
    //TRNG->CR.reg = (1 << 2);
    TRNG->CR.field.RNGEN = 1; // Enable TRNG
    ...
    while (1) {
        ...
        if (now >= next_tick) {
            ...
            while (!TRNG->SR.field.DRDY) {
            };

            printf("Tick %lu ( loop = %lu rnd = 0x%08x )\r\n", now / 1000, loop_cnt, TRNG->DR.reg);
