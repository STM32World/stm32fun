...
int main(void)
{
    ...
    if (lfs_file_open(&littlefs, &file, "speed.dat", LFS_O_RDWR | LFS_O_CREAT) == LFS_ERR_OK) {

        uint8_t buf[256];
        for (int i = 0; i < 256; ++i) buf[i] = i; // Fill the buffer

        lfs_file_rewind(&littlefs, &file);

        uint32_t start = uwTick;

        for (int i = 0; i < 2 * 4 * 1024; ++i) {
