 ...
 * @brief          : TinyUSB USB descriptors with direct MCU UID Serial Number
 ******************************************************************************
 * @attention
 *
...
char const *string_desc_arr[] =
        {
                ...
                NULL                           // 3: Serial Number (Generated dynamically from MCU UID)
        };
...
uint16_t const* tud_descriptor_string_cb(uint8_t index, uint16_t langid)
{
    ...
    if (index == 0)
    ...
    else if (index == 3) // Dynamic Serial Number directly from MCU UID
            {
        uint8_t const *id = (uint8_t const*) UID_BASE; // Standard HAL macro for UID registers

        // Convert 12 UID bytes (96 bits) directly to 24 UTF-16 hex characters
        for (uint8_t i = 0; i < 12; i++)
            ...
            _desc_str[1 + (i * 2)] = (high < 10) ? ('0' + high) : ('A' + high - 10);
            _desc_str[1 + (i * 2) + 1] = (low < 10) ? ('0' + low) : ('A' + low - 10);
        }
        chr_count = 24;
    }
    else
