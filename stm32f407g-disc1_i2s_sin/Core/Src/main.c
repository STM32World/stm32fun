...
float angle_change[2] = {
        60.0 * (TAU / SAMPLE_FREQ), // left
        30.0 * (TAU / SAMPLE_FREQ)  // right
};
...
void process_buffer(uint16_t *buffer) {
    ...
    for (int i = 0; i < 2 * I2S_DMA_BUFFER_SAMPLES; i += 2) { // Two samples left/right per step
        ...
        buffer[i] = OUTPUT_MID + OUTPUT_MID * amplification[0] * arm_sin_f32(angle[0]);      // Left
        buffer[i + 1] = OUTPUT_MID + OUTPUT_MID * amplification[1] * arm_sin_f32(angle[1]);  // Right
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    while (1) {
        ...
        if (now >= next_tick) {
            printf("Tick %lu (l=%lu ch=%lu cf=%lu)\n", now / 1000, loop, i2c_hcb, i2c_cb);
            loop = 0;
            next_tick = now + 1000;
        }
